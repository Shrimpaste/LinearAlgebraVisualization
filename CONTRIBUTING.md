...
换基语义必须写清矩阵所在坐标。一般 `V → W` 变换允许定义域基 `B` 与陪域基 `C` 不同，若 `A = [T]_{C←B}`，标准映射为 `T = C A B⁻¹`。eigensystem 只属于自同态 `T: V → V`，必须用同一个 `B` 得到相似矩阵 `B⁻¹AB`；不得用不同 `B/C` 声称保持特征值或特征向量。顺序复合必须注明 `T₂∘T₁` 中 `T₁` 先作用，并让 Canvas 与 Three.js 使用相同的 `I → T₁ → T₂T₁` 时间轴。
...
当前 schema 基线是线性变换 v3、内积 v2、向量张成/特征系统/谱分解/矩阵分解 v1；行列式仍未版本化。提升版本时必须保留所有已知旧版本迁移，并同时更新模型测试、部署回滚说明和本段基线。
...
- 向量组检查 1/6 个向量、冗余与共线输入、全部系数编辑，以及 `basisIndices` 的稳定顺序。
- 顺序复合检查 `T₁` 先作用、最终矩阵为 `T₂T₁`，并在 Canvas/Three.js 时间轴中点检查 `T₁` 状态。
- 特征换基检查 `B⁻¹AB` 与标准矩阵具有相同特征值，实特征向量坐标按 `B⁻¹` 转换；无效 `B` 不产生坐标矩阵，界面不提供不同输入/输出基。
