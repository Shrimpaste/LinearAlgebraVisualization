...
- `src/math` 只放无浏览器依赖的纯计算，不读取 React、DOM 或 Canvas 状态。
...
修改共享的 math、engine 或 rendering API 时，必须先搜索全部调用方并保持场景行为兼容；如果需要破坏性变更，应在同一 Pull Request 内迁移所有场景并更新架构文档。共享绘图改动至少检查五个场景的桌面和移动画布，控件改动同时检查键盘焦点、可读标签和禁用状态。

二维向量使用只读元组 `[x, y]`，二维矩阵使用行优先 `[m00, m01, m10, m11]`。普通退化情况通过可辨识联合类型返回，不使用异常控制流程；浮点判断应使用与输入尺度相关的容差。

## 新增场景

1. 在 `src/scenes/<id>/model.ts` 定义状态、默认值、预设和纯派生函数。
2. 使用 `SceneLayout` 与 `VisualizationStage` 创建场景组件。
3. 复用现有输入、范围、分段、状态和公式组件。
4. 将通用几何能力放入 `src/rendering`，不要把底层绘图复制到场景。
5. 在 `src/app/sceneRegistry.tsx` 注册场景元数据与懒加载组件。
6. 添加数值边界单测和至少一条完整用户路径的端到端验收。

更多职责和性能约束见[架构说明](docs/ARCHITECTURE.md)。

## 验收清单

- `npm run verify:full` 全部通过，浏览器控制台无错误或警告。
...
`quality` 工作流会在 push 和 Pull Request 上运行格式检查、TypeScript、零警告 ESLint、55 项单元/组件测试、生产构建以及桌面/移动端 Playwright 测试。只有全部检查通过后才应合并到 `main`；合并后由独立的 Pages 工作流发布。
