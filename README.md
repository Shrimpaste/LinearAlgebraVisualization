...
| 场景     | 可视化内容                                | 交互与边界处理                                                       |
| -------- | ----------------------------------------- | -------------------------------------------------------------------- |
| 向量张成 | 1–6 个 `R¹`、`R²` 或 `R³` 向量的秩与组合  | 直接编辑全部向量/系数，按输入顺序显示最大线性无关组                  |
| 线性变换 | `Rⁿ → Rᵐ`，`m,n ∈ {1,2,3}`                | 编辑 `A = [T]_{C←B}`；方阵可顺序复合 `T₂∘T₁`，时间轴展示 `I→T₁→T₂T₁` |
| 特征系统 | `R¹`、`R²` 或 `R³` 自同态的认证实特征方向 | 在共享基 `Q` 下读取 `Q⁻¹AQ`，显示实特征基 `P` 与 `[T]_P = P⁻¹AP`     |
| 内积空间 | `R/C`、1–3 维 Gram 内积、投影与正交化     | 编辑 `G`，逐项验证正定性、第一槽加性/齐性和共轭对称性                |
| 行列式   | `R²` 有向面积、定向翻转和坍缩             | 播放列变换，解释缩放、换列与加列操作                                 |
| 谱分解   | `R/C`、1–3 维自伴与 normal 算子           | 验证 `A=UΛU*`，真实演示 `x→U*x→Λc→UΛc=Ax`，按谱投影处理重根          |
| 矩阵分解 | 1–3 行/列实矩阵的 SVD 与右极分解          | 同步演示 `Vᵀ → Σ → U` 与 `P → Q → QP`，标明秩亏和部分等距            |

| 解集与拟合 | 1–3 行/列实矩阵的 Ax=b 与最小二乘 | 显示最小范数解、零空间自由参数、投影点与正交残差 |

一般线性变换可以连接不同空间：若 `A = [T]_{W←V}`，定义域有序基为 `V=(v₁,…,vₙ)`、陪域有序基为 `W=(w₁,…,wₘ)`，则标准坐标映射是 `T = W A V⁻¹`。切换标准基/自定义基或方阵的 `W=V` 选项时，编辑矩阵会自动换算以保持实际映射不变。特征系统只讨论自同态 `T: V → V`；同一基 `Q` 同时用于输入和输出，标准矩阵 `A` 的坐标表示是相似矩阵 `[T]_Q = Q⁻¹AQ`。使用不同输入/输出基不保持 eigensystem，因此特征场景不提供这种伪换基。

所有场景共享确定性时间轴、暂停/重播/定位、PNG 导出、浅色/深色主题及本地状态恢复；Canvas 舞台还提供速度控制。线性变换的顺序复合仅适用于方阵，Canvas 与 Three.js 舞台都把时间轴分为 `I → T₁ → T₂T₁` 两段。状态 schema 当前为：线性变换 v5、内积 v2、向量张成 v2、特征系统 v2、谱分解 v2、矩阵分解 v1；只有行列式仍使用未版本化状态。

一维与二维实验使用高 DPI Canvas 2D；张成、线性变换、实数内积、特征系统、实数谱分解和矩阵分解在三维时按需加载 Three.js，并提供轨道相机、PNG 导出、WebGL 恢复和无 WebGL 后备状态。实数三维内积通过 `SᵀS=G` 的等距坐标显示度量几何；实 normal 非自伴算子以实不变平面展示，复内积与复谱场景继续使用明确标注的分量/相位投影。界面支持键盘焦点、Canvas 文本替代、响应式移动布局和减少动态效果偏好。
...
本地优化分支增加八场景「入门引导 / 自由实验」，按预测、操作、观察、解释组织学习；自由实验保留原有参数。张成目标练习、特征方向预测、行列式预测与解集问题提供即时反馈。移动端保持画布可见，参数面板独立滚动。

- 「撤销 / 重做」记录当前场景会话中的最多50组修改，连续400ms内的编辑合并；键盘使用 Ctrl/Cmd Z 与 Shift Z。切换场景后保留参数，撤销历史重新开始。
- 「保存快照」在当前浏览器保存最多20个命名实验；载入快照恢复参数与视图。
- 「分享当前实验」链接携带版本化参数、动画进度和二维视口或三维相机。顶部复制按钮仍分享场景入口。剪贴板需安全来源（HTTPS或localhost）与浏览器支持。
- 「导出参数 JSON / 导入实验」支持离线交换，导入先验证格式、维数、数域及视图；不兼容数据会提示原因。文件上限64KB。
- 「带入矩阵」携带标准坐标中的实际映射到变换、特征、行列式、谱分解、SVD或求解器；矩形与复数不兼容时明确提示。求解器的目标b独立于映射，跨模块时初始化为全1。
- 「导出教学图卡」在画布之外附上中文参数、阶段与观察说明；原有纯画布PNG导出继续可用。
- 只读数值显示6位有效数字，可展开完整精度；编辑与计算保留原始精度。

解集模块使用相对容差SVD判定数值秩，给出 `x = x* + Σtᵢnᵢ`；最小二乘残差垂直于列空间，零参数对应最小范数解。接近退化时应同时查看秩和残差证书。新增功能的实施与验收见[优化记录](docs/OPTIMIZATION.md)。此处描述本地源码能力，在线版本以实际发布为准。
