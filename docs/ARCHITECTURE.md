...
原有张成、特征系统和行列式场景继续使用经过充分验证的 `R²` 类型：

- `Vec2` 是只读元组 `[x, y]`；
- `Mat2` 是行优先元组 `[m00, m01, m10, m11]`；
- 张成场景接受 1–6 个向量，以输入顺序执行尺度归一的贪心秩增长，返回稳定的 `basisIndices`、rank 和张成分类；
- 特征系统只处理自同态 `T: V → V`：标准矩阵 `A` 在单一基 `B` 下表示为相似矩阵 `[T]_B = B⁻¹AB`，不能用独立定义域/陪域基伪造 eigensystem 换基；
...
- 若 `A = [T]_{C←B}` 是从定义域基 `B` 坐标到陪域基 `C` 坐标的坐标矩阵，则对应的标准坐标映射统一为 `T = C A B⁻¹`；
...
输入或输出维数为 3 时，变换与矩阵分解按需加载 `ThreeTransformStage`。它把 1D/2D 对象嵌入 `R³`；单步模式按 `(1-t)x + tTx` 变形，复合模式与 Canvas 一致经过 `I → T₁ → T₂T₁`，避免把不兼容的定义域/陪域伪装成同一坐标系。
...
`useLocalStorage` 在初始化时调用场景迁移器，再持久化规范化结果。迁移器必须接受未知 JSON，并按明确的版本分支处理：缺失版本或已知旧版本执行受支持的迁移，当前版本重新校验并规范化，未来版本或其他不识别版本回退到当前默认值；所有分支都要验证形状与有限数。当前场景 schema 为：线性变换 v3、内积 v2、向量张成/特征系统/谱分解/矩阵分解 v1；只有行列式仍未版本化。矩阵编辑器的表单复数 `{ real, imag }` 只存在于 UI 边界，进入数学层前显式转换为 `{ re, im }`。
...
- **Vitest**：矩阵不变量、向量组贪心选基、一般换基与同基相似换基、顺序复合、Gram 公理、normal 分类、谱残差、SVD/极分解、迁移、视口和时间轴。
- **Playwright**：七场景导航、1–6 向量编辑/自动基、`T₂∘T₁` 的 Canvas/Three 中间状态、特征单基换基、正反变换、R/C 切换、3D readPixels、PNG IHDR、主题、响应式及控制台错误/警告。
