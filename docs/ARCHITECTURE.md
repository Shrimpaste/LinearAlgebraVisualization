...
- normal 谱分解只有在 normal 性、正交性、特征方程和重构残差全部过门槛时才返回成功；场景模型进一步计算 `c=U*x`、`d=Λc`、各贡献 `dᵢuᵢ`、`Ax` 应用残差和按重根分组的唯一谱投影 `Pλ`；
...
输入或输出维数为 3 时，变换按需加载 `ThreeTransformStage`。它把 1D/2D 对象嵌入 `R³`；单步模式按 `(1-t)x + tTx` 变形，复合模式与 Canvas 一致经过 `I → T₁ → T₂T₁`，避免把不兼容的定义域/陪域伪装成同一坐标系。张成、实数内积和特征系统分别按需加载 `ThreeSpanStage`、`ThreeInnerProductStage` 和 `ThreeEigenStage`；三维内积以满足 `SᵀS=G` 的坐标映射绘制 `Sx`，使画面中的欧氏长度、夹角与正交关系忠实对应原 `G` 内积。特征舞台只绘制认证的实特征几何，复特征值不会伪装为实方向。
...
`useLocalStorage` 在初始化时调用场景迁移器，再持久化规范化结果。迁移器必须接受未知 JSON，并按明确的版本分支处理：缺失版本或已知旧版本执行受支持的迁移，当前版本重新校验并规范化，未来版本或其他不识别版本回退到当前默认值；所有分支都要验证形状与有限数。当前场景 schema 为：线性变换 v5、内积 v2、向量张成 v2、特征系统 v2、谱分解 v2、矩阵分解 v1；只有行列式仍未版本化。矩阵编辑器的表单复数 `{ real, imag }` 只存在于 UI 边界，进入数学层前显式转换为 `{ re, im }`。
